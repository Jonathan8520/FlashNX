...
- **The launcher opens about three times faster.** The black screen before your games appear went from roughly 3.6 seconds to 1.1 on a 71-game library. The grid was preparing the cover of every single game before it could draw anything, so it now prepares only the tiles you can actually see; the SD card was being asked about companion files that were not there, which is now answered from the directory listing the scan already read; and the launcher was reserving 576 MB of graphics memory that only a game ever uses.

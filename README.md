...
| 1.4 ✓ Backends : `SwitchLogBackend` (→ nxlink) + Null defaults pour audio/video/nav/ui/storage (validé hardware 2026-05-24) | ~30 min | **Résolu**. `ruffle_core` fournit déjà `NullNavigatorBackend`, `NullUiBackend`, `MemoryStorageBackend`, `NullAudioBackend`, `NullVideoBackend` ; `PlayerBuilder::build()` les utilise par défaut. On écrit juste `SwitchLogBackend` qui route `avm_trace`/`avm_warning` à `ruffle_log_cstr` pour voir les `trace("...")` du SWF dans nxlink. Stub `sysconf()` ajouté dans `cpp/src/ruffle_bridge.cpp` (newlib n'a pas la fn POSIX, mais `jpeg_decoder` la veut via `std::thread::available_parallelism()`) — retourne 1, le worker tombe sur son fallback single-thread. |
| 1.5.a ✓ Player.build() + tick/render chaque frame (empty movie validé hardware 2026-05-24) | ~2 h | **Résolu**. `ruffle_init` construit `PlayerBuilder` avec notre renderer + log backend + autoplay + viewport, appelle `.build()` (qui instancie AVM1+AVM2+GC arena+Stage+display object hierarchy+Mutex<Player>+Arc::new_cyclic). `ruffle_render_frame` fait `player.lock().unwrap().tick(FloatDuration::from_secs(1/60)) + .render()`. Avec empty movie : écran blanc (Ruffle clear par défaut). Confirme que la machinerie complète tourne sur ARM64 Horizon. `.a` 674 KB → 12.9 MB (+12 MB AVM + parsers) ; `.nro` 5.95 → 11.6 MB. Panic hook installé pour router vers nxlink (panic = abort donc déclencheur unique mais le message sort avant). |
| 1.5.b ✓ Real SWF parser + render (validé hardware 2026-05-24) | ~30 min | **Résolu**. `std::fs::read("sdmc:/switch/ruffle/test.swf")` marche (sdmc auto-monté par libnx crt0) ; en l'absence du fichier on tombe sur un fallback `include_bytes!("../../third_party/ruffle/swf/tests/swfs/SimpleRedBackground.swf")` (43 octets). `SwfMovie::from_data` parse correctement sur Switch (premier parsing SWF sur ARM64 Horizon dans ce projet) — log : `SwfMovie parsed (version=6, dims=400x400)`. Visuel confirmé : **fond rouge à l'écran** — toute la chaîne `SetBackgroundColor tag → Player.run_frame → Player.render → CommandList → submit_frame(clear=red)` fonctionne. |
| 1.5.c File picker C++ : scan `sdmc:/switch/ruffle/*.swf`, UI de sélection joycon, passer le path à Rust | 2-3 jours | Faible |

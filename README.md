...
1. Copier tes `.swf` sur la SD dans **`sdmc:/ruffle/`** (ou `sdmc:/switch/ruffle/`). N'importe quel nom de fichier marche. Depuis Phase 3.4 le `.nro` ouvre la **library FlashNX** au boot : liste de tous les `.swf` détectés, navigue avec haut/bas, **A** lance, **X** ouvre OPTIONS (TOUCHES pré-launch), **-** quitte le `.nro`. Si aucun `.swf` n'est trouvé, l'écran « AUCUN JEU » t'indique où en poser.
   - Empty state : la library te montre où poser les fichiers, **-** referme le `.nro`.
   - Fallback ultime (library init fail) : `SwitchRenderBackend::new` foire → ruffle_init avec le SWF embarqué 43-octet (fond rouge). Pas d'UI, juste une preuve que le `.nro` peut booter.
...
| 3.4 ✓ Library UI ("FlashNX launcher") | **Livré 2026-05-26 nuit** ([rust/src/library.rs](rust/src/library.rs) + [rust/src/backend/render.rs](rust/src/backend/render.rs) `draw_library_*` + [cpp/src/swf_picker.cpp](cpp/src/swf_picker.cpp) + [cpp/src/main.cpp](cpp/src/main.cpp) boot flow). C++ scan SD via `opendir`/`readdir` newlib (contourne le bug Rust `read_dir` Horizon) sur les deux candidates `sdmc:/ruffle/*.swf` + `sdmc:/switch/ruffle/*.swf`, push chaque path dans le Rust library state via `ruffle_library_add_path`. Rust parse l'entête SWF inline : compression (FWS/CWS/ZWS), version SWF, dims via RECT en bits MSB-first — `flate2::read::ZlibDecoder` pour les CWS (déjà dans le tree via Ruffle, binary cost ~0). **Boot flow** : `gl_context_init` → `ruffle_library_init` (alloue un `SwitchRenderBackend` standalone hors-Player) → `swf_picker_run` → `ruffle_library_open` → boucle input/render jusqu'à pick ou quit. À pick : `ruffle_set_swf_path(selected)` + `ruffle_library_shutdown` (drop standalone renderer pour libérer ~96 MB arena GPU **avant** que `ruffle_init` n'alloue le sien) → `ruffle_init` → game loop normal. À quit : exit `.nro` directement (sans jamais initialiser Ruffle). **Layout** : banner PNG `assets/banner.png` en haut (uploadé en texture via crate `png` 0.18 + `upload_rgba_texture`), liste de 6 rows scrollables avec curseur `>` animé `sin(time)`, color chip 16×16 px par jeu (HSV hash basename → couleur stable), ligne sélectionnée pulse amber↔bright-amber. Metadata panel en bas (display_name + `<size> // SWF V<version> <compression> // <WxH>` + `[basename.swf]` en petit pour ne jamais perdre l'info du fichier physique). Fallback ASCII "FLASHNX" avec drop shadow si le banner decode échoue. **Inputs** : **A**=JOUER, **X**=OPTIONS, **-**=QUITTER, Up/Down/StickL pour naviguer. **Empty state** quand SD vide → instructions où poser les `.swf` + drop shadow titre "AUCUN JEU". **Modal OPTIONS** = TOUCHES + RETOUR (TOUCHES réutilise `menu::open()` via `keymap::init_for_swf(basename)` pour cibler le sidecar du jeu sélectionné AVANT lancement — permet de configurer les touches sans jamais booter Ruffle). **Hors scope v1 (Phase 3.4.bis)** : Renommer (**display-name only** via sidecar `.meta.json` `{"display_name": "..."}` + libnx `swkbd` — **on NE renomme PAS le fichier `.swf` ni rien d'autre**. Le basename physique reste, donc `.sol`/`.keymap.json`/URLs Ruffle/SharedObject paths inchangés → zéro migration en cascade, zéro risque de saves perdues. Pattern Steam/ScummVM/iTunes). Supprimer (avec confirm, préserve `.sol`), tri, jaquettes (skip décidé — user passe par un forwarder Sphaira pour une icône custom sur home menu, cf. notes ban sysNAND ci-dessous), grid view, search. **Pas de "Back to library"** depuis le pause menu au v1 (QUITTER exit le `.nro`). Remplace définitivement Phase 2.6 (file picker hardcodé). | ✓ |
...
| 3.7 Import distant (mode local ↔ archive.org) | **Design arrêté 2026-05-26 nuit, code à venir**. Ajout d'un toggle « LOCAL / DISTANT » dans la library : en mode DISTANT, l'utilisateur colle une URL d'item [archive.org](https://archive.org/details/) (ex. `https://archive.org/download/banned-from-equestria-daily-1.5`) via le clavier libnx `swkbd`. On hit `https://archive.org/metadata/<item-id>` (API JSON officielle, stable, retourne `{files: [{name, format, size}, ...]}` ; on filtre `format == "Shockwave Flash"`). Modal de confirmation « N fichiers trouvés », case par fichier, **A** = télécharger en `sdmc:/ruffle/<basename>`. **Stack technique** : libcurl + mbedtls (`switch-curl` + `switch-mbedtls` via pacman devkitPro), bundle CA Mozilla `cacert.pem` embarqué via `include_bytes!` car `/etc/ssl/certs` n'existe pas sur Horizon. JSON parse via `serde_json` (déjà direct dep pour keymap). Progress bar = `glDrawArrays` solid_prog par fichier. À la fin, basculer auto retour en mode LOCAL avec le nouveau jeu sélectionné. **Hors scope v1 de 3.7** : URLs génériques non-archive.org (HTML scraping, MIME sniffing) — phase 2 si demande, archive.org couvre ~95 % de la préservation Flash actuelle. **Estimation** : ~3-4 j focused (infra HTTPS + JSON + UI mode toggle + download progress + écrire dans `sdmc:/ruffle/`). **Risques** : (a) WiFi Switch ~2-5 MB/s typique, un `banned-from-equestria-daily-1.5` ~30-50 MB descend en ~10-20s acceptable, (b) bundle CA ajoute ~250 KB au `.nro`, (c) libcurl + mbedtls statiques ajoutent ~1-2 MB au `.nro`. | Moyen, ~3-4 j |

### Phase 4 — polish + distribution

- ✓ **`.nacp` + icône custom embarqués 2026-05-26 nuit** : `cpp/Makefile` wired avec `APP_TITLE = "FlashNX"`, `APP_AUTHOR = "flash-for-switch contributors"`, `APP_ICON = $(TOPDIR)/../assets/icon.jpg`, `NROFLAGS += --nacp=... --icon=...`. hbmenu / Sphaira affichent désormais le bon titre + icône (plus de "Unknown" question-mark).

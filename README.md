...
| 1.3.1 ✓ `SwitchRenderBackend` squelette validé hardware (2026-05-23) | ~3 h | **Résolu**. `rust/src/backend/render.rs` implémente les 15 méthodes requises du trait `ruffle_render::backend::RenderBackend` + 4 default impls. `submit_frame` exécute `glClearColor`/`glClear` à partir de `swf::Color` ; `set_viewport_dimensions` appelle `glViewport`. Le reste est Null-style (`Ok(BitmapHandle::new(SwitchBitmapHandle))` ou `Err(Unimplemented(...))`) — suffisant pour que `PlayerBuilder::with_boxed_renderer` accepte le renderer sans crasher. `ruffle_init` exerce le trait via un `Box<dyn RenderBackend>` (`name`, `debug_info`, `submit_frame`) pour défaire la dévirtualisation LTO. `nm` confirme les 19 symboles présents dans `libruffle_switch.a`. **Hardware OK** : nxlink stdout montre `phase 1.3: renderer=switch-mesa-gl (Renderer: SwitchRenderBackend (phase 1.3 iter 1, clear-only))` puis `ruffle_init: triangle pipeline ready` puis exit propre via `+`. Aucun nouveau patch stdlib requis. `.nro` = 5.85 MB (+8 KB vs Phase 1.2). |
| 1.3.2 Mapper `CommandList` → triangles tesselés → `glDrawArrays` (parcours du command list, cache shape par `ShapeHandle`) | 2-4 semaines | Moyen |
| 1.3.3 Upload bitmap réel via `glTexImage2D`/`glTexSubImage2D` dans `register_bitmap`/`update_texture` | 3-5 jours | Faible |
| 1.3 pivot si wgpu-GL casse sur switch-mesa : déjà engagé en GL natif, donc rien à pivoter — c'est cette implémentation. |  |  |
